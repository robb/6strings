...
DEBUG = true;(function() {
  var HALF_PI, PI, Synthesizer, TWO_PI, clip;
  ...
  Synthesizer = (function() {
    function Synthesizer() {
      ...
      this.node = this.context.createJavaScriptNode(512);
      this.strings = (function() {
        var _results;
        _results = [];
        for (i = 0; i <= 88; i++) {
          _results.push(new Synthesizer.String(i));
        }
        return _results;
      })();
      this.node.onaudioprocess = __bind(function(event) {
        var channels, frame, outputArrayL, outputArrayR, sample, string, _i, _len, _ref, _ref2, _results;
        outputArrayL = event.outputBuffer.getChannelData(0);
        outputArrayR = event.outputBuffer.getChannelData(1);
        _results = [];
        for (frame = 0, _ref = outputArrayL.length; 0 <= _ref ? frame <= _ref : frame >= _ref; 0 <= _ref ? frame++ : frame--) {
          channels = [0, 0];
          _ref2 = this.strings;
          ...
          _results.push(outputArrayR[frame] = channels[1]);
        }
        return _results;
      }, this);
  ...
  Synthesizer.Lowpass = (function() {
    ...
    Lowpass.prototype.apply = function(sample) {
      var result;
      result = this.gain * (sample + this.previous);
      this.previous = sample;
      return result;
    };
    return Lowpass;
  })();
  Synthesizer.String = (function() {
    function String(pitch, samplerate) {
      ...
      loopLength -= 0.5;
      this.L = Math.floor(loopLength);
      maxlength = 1;
      while (maxlength <= this.L) {
        maxlength <<= 1;
      }
      this.delayline = new Array(maxlength);
